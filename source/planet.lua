...
function Planet:setName(name) self.name = name end

function Planet:setRadius(rad) self.radius = rad end

function Planet:setScale(scale) self.scale = scale end

function Planet:setRevolution(rev) self.revolution = rev end

function Planet:setDays(days) self.days = days end

function Planet:setSelected(sel) self.selected = sel end

function Planet:setPitch(pitch) self.pitch = pitch end

function Planet:getName() return self.name end

function Planet:getRevolutions() return self.days / self.revolution end

function Planet:getPitch() return self.pitch end
...
function Planet:draw()
	local x <const> = halfScreenWidth + math.sin(self.scaledDays * tau) *
		self.radius
	local y <const> = halfScreenHeight - math.cos(self.scaledDays * tau) *
		self.radius
